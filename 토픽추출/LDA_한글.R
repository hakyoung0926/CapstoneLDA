...
library(MASS)
# KoNLP 사용을 위한 
path <- file.path("C:/Users/안운빈/Desktop/3-2/알고리즘/LDA/txt")  # 폴더 경로를 객체로 만든다
kor <- list.files(file.path(path, "2019"))  #폴더 경로 중 eng라는 폴더에 있는 파일들 이름을 list-up해서 객체로 만든다.
kor.files <- file.path(path, "2019", kor)   #아까 list-up한 파일의 이름들로 폴더의 경로를 다시 객체로 만든다.
...
topic <- gsub("한국산업기술대학교", "", topic)
topic <- gsub("이용", "", topic)
topic <- gsub("사용", "", topic)
topic <- gsub("한국정보통신학회논문", "", topic)
topic <- gsub("방법", "", topic)
topic <- gsub("기법", "", topic)
topic <- gsub("논문", "", topic)
topic <- gsub("연구", "", topic)
