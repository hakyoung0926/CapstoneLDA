...
setwd("C:/Users/KHY/Documents/GitHub/CapstoneLDA") #불러들일경로 (negative,positive.txt여기에있어야함)
#txt<-readLines("15.txt",warn=FALSE) #감정분석할 텍스트파일 불러오기

path <- file.path("C:/Users/KHY/Documents/GitHub/CapstoneLDA")  # 폴더 경로를 객체로 만든다
kor <- list.files(file.path(path, "아몬드"))  #폴더 경로 중 eng라는 폴더에 있는 파일들 이름을 list-up해서 객체로 만든다.
kor.files <- file.path(path, "아몬드", kor)   #아까 list-up한 파일의 이름들로 폴더의 경로를 다시 객체로 만든다.
content<- readLines(file.path(path,"아몬드/content.txt"))          # content를 따로 저장
summa <- "줄거리"
text_topic <- "토픽 분석"
text_sentiment <- "감정 분석" 
titleName <- "리뷰 분석 시스템"
...
new_sentiment <- readLines("emotion.txt")
...
ui <- fluidPage(
  tags$style(HTML("
      ...
      html > body > .container-fluid > h2 > .shiny-text-output{
...
#  theme = shinytheme("united"),
  # App title ----
  titlePanel(textOutput("titleName")),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      tabsetPanel(
                  tabPanel(textOutput("text_topic"), sliderInput("nTerms", "Number of terms to display", min = 20, max = 40, value = 30), visOutput('myChart')),
                  tabPanel(textOutput("text_sentiment"), 
                           splitLayout(
...
server <- shinyServer(function(input, output, session) {
  ...
  output$titleName <- renderText(titleName)
  output$text_topic <- renderText(text_topic)
  output$text_sentiment <- renderText(text_sentiment)
  output$summa <- renderText(summa)
