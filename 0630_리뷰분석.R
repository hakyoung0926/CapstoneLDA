...
path <- file.path(dirname(rstudioapi::getSourceEditorContext()$path))  # 폴더 경로를 객체로 만든다
kor <- list.files(file.path(path, bookName))  #폴더 경로 중 eng라는 폴더에 있는 파일들 이름을 list-up해서 객체로 만든다.
kor.files <- file.path(path, bookName, kor)   #아까 list-up한 파일의 이름들로 폴더의 경로를 다시 객체로 만든다.
content<- readLines(file.path(path,bookName,"content.txt"))          # content를 따로 저장
summa <- "줄거리"
text_topic <- "토픽 분석"
...
new_sentiment <- readLines("sentiment.txt")
...
ui <- fluidPage(
  ...
  # App title ----
  titlePanel(strong(textOutput("titleName"))),
  
  # Sidebar layout with input and output definitions ----
  sidebarLayout(
    # Sidebar panel for inputs ----
    sidebarPanel(
      h2(textOutput("bookName"),align = "center"),
      hr(),
      h3(textOutput("summa"),align = "center"),
      hr(),
      textOutput("content"),
      br()),
    ...
    mainPanel(
      ...
      tabsetPanel(
        tabPanel(textOutput("text_topic"), #sliderInput("nTerms", "Number of terms to display", min = 20, max = 40, value = 30),
                 visOutput('myChart')),
